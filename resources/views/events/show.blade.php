<x-layout background="events-bg">
    <div class="max-w-4xl mx-auto py-8">
        <div class="mb-6">
            <a href="{{ route('events.index') }}" class="text-white hover:underline font-medium">&larr; Back to Events</a>
        </div>

        <div class="glass-climbing rounded-2xl overflow-hidden">
            <div class="p-8">
                ...
                <div class="border-t border-slate-200 pt-6">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        @auth
                            <form action="{{ route('events.join', $event) }}" method="POST">
                                @csrf
                                @if($event->users->contains(auth()->user()))
                                    <button type="submit" class="bg-red-100 text-red-600 border border-red-600 px-4 py-2 rounded-lg hover:bg-red-200 transition">
                                        Leave Event
                                    </button>
                                @else
                                    <button type="submit" class="bg-primary-500 text-white px-6 py-2 rounded-lg hover:bg-primary-600 transition shadow">
                                        Join Event
                                    </button>
                                @endif
                            </form>
                        @else
                            <a href="{{ route('login') }}" class="text-primary-500 hover:underline">Login to join</a>
                        @endauth
                    </div>
                    ...
                    @if($event->users->count() > 0)
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            @foreach($event->users as $user)
                                <div class="flex items-center space-x-3 p-3 bg-white/50 rounded-xl">
                    ...
                    @else
                        <p class="text-slate-500 italic">No climbers have joined yet. Be the first!</p>
                    @endif
                </div>
            </div>
        </div>
