<x-layout>
    <div class="flex justify-between items-center mb-6">
        ...
        @auth
            <a href="{{ route('events.create') }}" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">+ Create Event</a>
        @endauth
    </div>
        ...
        @foreach($events as $event)
            <div class="bg-white p-6 rounded-lg shadow flex justify-between items-center">
                <div>
                    ...
                    <p class="text-gray-600"><span class="font-semibold">Date:</span> {{ $event->date->format('l, F j, Y \a\t H:i') }}</p>
                    ...
                    @auth
                        <form action="{{ route('events.join', $event) }}" method="POST">
                            ...
                            @if($event->users->contains(auth()->user()))
                                <button type="submit" class="bg-red-100 text-red-600 border border-red-600 px-4 py-2 rounded hover:bg-red-200">
                                    Leave Event
                                </button>
                            @else
